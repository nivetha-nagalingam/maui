...
public partial class WebViewOptionsPage : ContentPage
{
    ...
    private void UserAgentEntry_TextChanged(object sender, TextChangedEventArgs e)
    {
    ...
    private void OpacityRadio_CheckedChanged(object sender, CheckedChangedEventArgs e)
    {
        if (!(sender is RadioButton rb) || !rb.IsChecked)
            return;
        if (double.TryParse(rb.Content?.ToString(), out var opacity))
    ...
    private void CancelNavigationRadio_CheckedChanged(object sender, CheckedChangedEventArgs e)
    {
        if (!(sender is RadioButton rb) || !rb.IsChecked)
            return;
        _viewModel.CancelNavigation = rb.Content?.ToString() == "True";
    }
}